...
const clinicians = ["Barb", "Isaac", "Marisol", "Mary", "Shawna", "Shelly", "Tom"]; // represents model of clinician names and addresses
let distances = new Map() // format will be [clinician name, total distance of clinician's trip]
...
function calculateDistance(patientAddress, labBool) {
  if (labBool) {
    clinicians.forEach(
      (clinician) =>
        (distances[clinician] =
          clinicianToPatient(clinician.address, patientAddress) + clinicianToLab(clinician.address, clinician.labAddress))
    );
  } else {
    clinicians.forEach(
      (clinician) =>
        (distances[clinician] = clinicianToPatient(clinician.address, patientAddress))
    );
  }
}
